...
public class ValidateConnectNGame {
	...
	public static boolean checkName(String name) {
		if (name.length() > 0) // if the name is filled
			return true;
		return false;
	} // checkName(String)
	...
	public static boolean validateConnectNumber(int num, int row, int col) {
		boolean valid = false;
		if (num < 9 && num > 2 && (num <= row || num <= col)) // if num is within range of max AND min AND within range of row OR column
			valid = true;
		return valid;
	...
	public static boolean validateNumRowsCols(int num) {
		boolean valid = false;
		if (num < 13 && num > 3) // if num is within range of max and min
			valid = true;
		return valid;
	...
	public static int validateTokenDropped(int col, char[][] gameBoard) {
		if (col < 0 || col >= gameBoard[0].length) // if the column doesn't exist on the board
			...
			if (gameBoard[0][col] != ConnectNGame.PLAYER_CHARACTERS[2]) // if the top row isn't empty
				return -2;
		return 0;
